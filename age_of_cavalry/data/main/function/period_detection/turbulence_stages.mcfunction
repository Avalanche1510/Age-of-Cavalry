...
execute as @e[type=armor_stand,tag=record] if score @s last_stage < @s difficulty run scoreboard players set @s halt 300
execute as @e[type=armor_stand,tag=record] if score @s last_stage < @s difficulty run tellraw @a [{"text":"世界动荡等级上升，将暂时熔断！","color":"white"}]
execute as @e[type=armor_stand,tag=record] if score @s last_stage < @s difficulty run tellraw @a [{"text":"世界动荡值将于","color":"white"},{"score":{"name":"@s","objective":"halt"},"color":"red"},{"text":"秒后再次开始发生变化！","color":"white"}]
