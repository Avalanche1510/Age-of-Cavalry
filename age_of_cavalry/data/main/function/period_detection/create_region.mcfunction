...
execute as @a at @s unless score @s playerHealth matches 0 if dimension minecraft:overworld unless biome ~ ~ ~ #minecraft:is_ocean unless biome ~ ~ ~ terralith:deep_warm_ocean run execute if score @s region_y matches 63..250 run execute unless entity @n[type=marker,distance=..192,tag=region_center] run function main:region/random_region
