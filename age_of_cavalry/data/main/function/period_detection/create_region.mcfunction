execute as @a store result score @s region_y run data get entity @s Pos[1] 1
execute as @a at @s if dimension minecraft:overworld run execute if score @s region_y matches 63.. run execute unless entity @n[type=marker,distance=..192,tag=region_center] run function main:region/random_region
