...
execute as @n[type=armor_stand,tag=col,tag=initialized] at @s if score @s c_col = @s col run execute as @e[distance=..64,tag=soldier,tag=!has_attributes,tag=legend_mob] run function main:army/attribute_type/legend
execute as @n[type=armor_stand,tag=col,tag=initialized] at @s if score @s c_col = @s col run execute as @e[distance=..64,tag=soldier,tag=!has_items,team=illager_party] run function main:army/assign_items/assign_items_illager
